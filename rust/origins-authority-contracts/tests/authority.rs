...
fn scope() -> Value {
    fixtures()["valid"][0]["contract"].clone()
}
fn lease() -> Value {
    fixtures()["valid"][1]["contract"].clone()
}
fn apply(value: &mut Value, key: &str, item: Value) {
...
fn shared_v11_corpus_and_hashes() {
    ...
    for item in corpus["valid"].as_array().unwrap() {
        ...
        assert_eq!(
            authority_sha256(contract).unwrap(),
    ...
    for item in corpus["invalid"].as_array().unwrap() {
        ...
        assert_eq!(
            error.code,
            item["expected_error"].as_str().unwrap(),
            "{}",
            item["name"]
        );
    }
...
fn sec002_candidate_binds_once_and_operation_is_immutable() {
    ...
    assert_eq!(
        validate_child_scope(&operation_swap, &root)
            .unwrap_err()
            .code,
        "SCOPE_ESCALATION"
    );
    ...
    assert_eq!(
        validate_child_scope(&switched, &bound_parent)
            .unwrap_err()
            .code,
        "SCOPE_ESCALATION"
    );
...
fn sec004_scope_generation_is_fenced() {
    ...
    assert_eq!(
        validate_scope_current(&stale, &current).unwrap_err().code,
        "STALE_SCOPE"
    );

    apply(&mut current, "state", json!("revoked"));
    assert_eq!(
        validate_scope_current(&current, &current).unwrap_err().code,
        "SCOPE_UNUSABLE"
    );
}
...
fn sec005_endpoint_protocol_and_port_are_authority() {
    ...
    apply(
        &mut child,
        "network_endpoints",
        json!([{"protocol":"http","host":"support.example.com","port":443}]),
    );
    assert_eq!(
        validate_child_scope(&child, &scope()).unwrap_err().code,
        "SCOPE_ESCALATION"
    );
    ...
    apply(
        &mut invalid,
        "network_endpoints",
        json!([{"protocol":"https","host":"support.example.com","port":0}]),
    );
    assert_eq!(
        validate_authority_contract(&invalid).unwrap_err().code,
...
fn issuance_chronology_and_holder_generation_fail_closed() {
    ...
    assert_eq!(
        validate_authority_contract(&item).unwrap_err().code,
