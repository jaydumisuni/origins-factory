...
fn launcher_router(state: LauncherState) -> Router {
    Router::new()
        ...
        .route("/origins-intelligence", any(optional_mount_not_exposed))
        .route(
            "/origins-intelligence/*path",
            any(optional_mount_not_exposed),
        )
        .route("/origins-phase5", any(optional_mount_not_exposed))
        .route("/origins-phase5/*path", any(optional_mount_not_exposed))
        .route("/origins-phase6", any(optional_mount_not_exposed))
        .route("/origins-phase6/*path", any(optional_mount_not_exposed))
        .route("/origins-phase7", any(optional_mount_not_exposed))
        .route("/origins-phase7/*path", any(optional_mount_not_exposed))
...
async fn optional_mount_not_exposed() -> Response {
    no_store(
        (
            StatusCode::NOT_FOUND,
            Json(json!({
                "ok": false,
                "error": "OPTIONAL_MOUNT_NOT_EXPOSED",
                "message": "optional Origins services remain separate mounts in Windows v1 installed mode",
            })),
        )
            .into_response(),
    )
...
async fn bootstrap(
    ...
    if !valid {
        return no_store((StatusCode::UNAUTHORIZED, Json(json!({"ok": false, "error": "invalid_bootstrap_nonce", "installed_proxy": true, "authenticated": false}))).into_response());
    }
    *expected = None;
    let cookie = format!(
    ...
    match HeaderValue::from_str(&cookie) {
        ...
        Err(_) => no_store((StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"ok": false, "error": "cookie_encoding_failed", "installed_proxy": true, "authenticated": false}))).into_response()),
    }
}
...
async fn ensure_daemon(
    ...
    if health_ok(client, &health_url).await {
        let token = read_local_token(data_dir).map_err(|error| {
            format!(
                "healthy originsd already occupies {daemon_addr}, but token from intended data root {} is unavailable: {error}",
                data_dir.display()
            )
        })?;
        if daemon_accepts_local_token(client, daemon_addr, &token).await {
            return Ok(None);
        }
        return Err(format!(
            "healthy originsd already occupies {daemon_addr}, but it does not accept the token from intended data root {}",
            data_dir.display()
        ));
    }
    ...
    if !daemon_accepts_local_token(client, daemon_addr, &token).await {
        ...
        let _ = child.wait().await;
        return Err(format!(
            "started originsd at {daemon_addr}, but it did not accept the token from intended data root {}",
            data_dir.display()
        ));
    }
    Ok(Some(child))
...
async fn daemon_accepts_local_token(client: &Client, daemon_addr: SocketAddr, token: &str) -> bool {
...
async fn run_probe() -> Result<(), String> {
    ...
    match ensure_daemon(&client, &exe_dir, &mismatched_root, first_addr).await {
        Err(error) if error.contains("token from intended data root") => {}
        Ok(Some(mut unexpected)) => {
            let _ = unexpected.kill().await;
            let _ = first.kill().await;
            let _ = fs::remove_dir_all(&mismatched_root);
            return Err("installed Origins reused an occupied daemon without requiring the intended local token".to_owned());
        }
        Ok(None) => {
            let _ = first.kill().await;
            let _ = fs::remove_dir_all(&mismatched_root);
            return Err(
                "installed Origins reused an occupied daemon with an unavailable or mismatched intended token".to_owned(),
            );
        }
        Err(error) => {
            let _ = first.kill().await;
...
fn require_no_store(response: &reqwest::Response, label: &str) -> Result<(), String> {
    let value = response
        .headers()
        .get(reqwest::header::CACHE_CONTROL)
        .and_then(|header| header.to_str().ok());
    if value != Some("no-store") {
        return Err(format!("{label} did not emit Cache-Control: no-store"));
    }
    Ok(())
}
...
async fn exercise_installed_proxy(
    ...
    require_no_store(&unauth, "unauthenticated bootstrap status")?;
    ...
    require_no_store(&bootstrap, "bootstrap exchange")?;
    ...
    require_no_store(&replay, "bootstrap replay refusal")?;
    ...
    require_no_store(&protected, "authenticated proxy")?;

    let optional = client
        .get(format!("{base}/origins-phase5/v1/health"))
        .header(reqwest::header::COOKIE, cookie)
        .send()
        .await
        .map_err(|error| format!("probe optional mount boundary: {error}"))?;
    if optional.status() != reqwest::StatusCode::NOT_FOUND {
        server.abort();
        return Err(
            "Windows v1 installed mode unexpectedly exposed an optional service mount".to_owned(),
        );
    }
    require_no_store(&optional, "optional mount refusal")?;
