...
impl ArtifactRootPolicy {
    ...
    fn require_source(&self, value: &str) -> Result<PathBuf, StoreError> {
        ...
        let canonical = fs::canonicalize(&configured)
            .map_err(|error| StoreError::Io(format!("Artifact source cannot be resolved: {error}")))?;
...
pub fn router(state: ArtifactState) -> Router {
    Router::new()
        ...
        .route("/v1/artifacts/:artifact_id/content", get(get_artifact_content))
        .with_state(state)
}
...
async fn get_artifact(
    ...
    require_auth(&headers, &state.local_token)?;
    let artifact_id = normalize_uuid(&artifact_id, "artifact_id").map_err(ArtifactApiError::from_store)?;
    let artifact = load_artifact(&state.store, &artifact_id)
        .map_err(ArtifactApiError::from_store)?
        .ok_or_else(|| ArtifactApiError::new(StatusCode::NOT_FOUND, "ARTIFACT_NOT_FOUND", "Artifact not found"))?;
    Ok(Json(artifact.projection().map_err(ArtifactApiError::from_store)?))
}
...
async fn register_artifact(
    ...
    let owner = request.owner.trim().to_ascii_lowercase();
    if owner != "lumi" {
        return Err(ArtifactApiError::new(
            StatusCode::BAD_REQUEST,
            "UNSUPPORTED_ARTIFACT_OWNER",
            "Phase 5 Artifact registration currently accepts Lumi owner handoffs only",
        ));
    }
    ...
    if let Some(existing) = find_artifact_by_owner_ref(&state.store, &workspace_id, &owner, &owner_ref)
        .map_err(ArtifactApiError::from_store)?
    {
        return Ok((
            StatusCode::OK,
            Json(json!({
                "registered": true,
                "reused": true,
                "artifact": existing.projection().map_err(ArtifactApiError::from_store)?,
            })),
        ));
    }
...
async fn get_artifact_content(
    ...
    let artifact = load_artifact(&state.store, &artifact_id)
        .map_err(ArtifactApiError::from_store)?
        .ok_or_else(|| ArtifactApiError::new(StatusCode::NOT_FOUND, "ARTIFACT_NOT_FOUND", "Artifact not found"))?;
    let projection = artifact.projection().map_err(ArtifactApiError::from_store)?;
    let object_path = PathBuf::from(&artifact.object_path);
    let canonical_object = fs::canonicalize(&object_path).map_err(|error| {
    ...
    let expected_size = projection
        ...
        .ok_or_else(|| ArtifactApiError::new(StatusCode::SERVICE_UNAVAILABLE, "CORRUPT_STATE", "Artifact size missing"))?;
    let metadata = fs::metadata(&canonical_object).map_err(|error| {
        ArtifactApiError::new(StatusCode::SERVICE_UNAVAILABLE, "ARTIFACT_CONTENT_UNAVAILABLE", error.to_string())
    })?;
    ...
    let mut file = tokio::fs::File::open(canonical_object)
        .await
        .map_err(|error| ArtifactApiError::new(StatusCode::SERVICE_UNAVAILABLE, "ARTIFACT_CONTENT_UNAVAILABLE", error.to_string()))?;
    ...
    *response.status_mut() = StatusCode::OK;
    response.headers_mut().insert(
        ...
        HeaderValue::from_str(&expected_size.to_string())
            .map_err(|error| ArtifactApiError::new(StatusCode::SERVICE_UNAVAILABLE, "CORRUPT_STATE", error.to_string()))?,
    );
    response.headers_mut().insert(
        CONTENT_TYPE,
        HeaderValue::from_str(media_type)
            .map_err(|_| ArtifactApiError::new(StatusCode::SERVICE_UNAVAILABLE, "CORRUPT_STATE", "invalid Artifact media type"))?,
    );
    response.headers_mut().insert(
        CONTENT_DISPOSITION,
        HeaderValue::from_str(&disposition)
            .map_err(|_| ArtifactApiError::new(StatusCode::SERVICE_UNAVAILABLE, "CORRUPT_STATE", "invalid Artifact filename"))?,
    );
    Ok(response)
}
...
async fn materialize(source: &FsPath, object_root: &FsPath) -> Result<MaterializedObject, StoreError> {
    ...
    let result = async {
        ...
        loop {
            ...
            output
                ...
                .await
                .map_err(|error| StoreError::Io(format!("Artifact staging write failed: {error}")))?;
        }
        output
            .flush()
            .await
        ...
        tokio::fs::create_dir_all(&destination_dir)
            .await
            .map_err(|error| StoreError::Io(format!("Artifact object directory failed: {error}")))?;
            ...
            .map_err(|error| StoreError::Io(format!("Artifact object check failed: {error}")))?
        {
            ...
            if tokio::fs::try_exists(&object_path).await.unwrap_or(false) {
                ...
                return Err(StoreError::Io(format!("Artifact materialization failed: {error}")));
            }
        }
        ...
        fs::set_permissions(&object_path, permissions)
            .map_err(|error| StoreError::Io(format!("Artifact readonly protection failed: {error}")))?;
...
async fn verify_object(path: &FsPath, expected_sha256: &str, expected_size: u64) -> Result<(), StoreError> {
...
fn register_materialized(
        ...
        .optional()?
    {
        let artifact = load_artifact_connection(&transaction, &source_artifact_id)?
            .ok_or_else(|| StoreError::Corrupt("Artifact source points to missing Artifact".to_owned()))?;
        if artifact
            .projection()?
            .get("content_sha256")
            .and_then(Value::as_str)
    ...
    let canonical = canonical_json(&projection)
        .map_err(|error| StoreError::InvalidInput(format!("Artifact projection serialization failed: {error}")))?;
    let projection_sha256 = contract_sha256(&projection)
        .map_err(|error| StoreError::InvalidInput(format!("Artifact projection digest failed: {error}")))?;
    transaction.execute(
        ...
        params![
            ...
            workspace_id,
            materialized.sha256,
            i64::try_from(materialized.size_bytes).map_err(|_| StoreError::InvalidInput("Artifact size out of SQLite range".to_owned()))?,
...
fn update_source_count(
    ...
    let projection = artifact_projection(
        ...
        previous.get("filename").and_then(Value::as_str).unwrap_or("artifact.bin"),
        previous.get("media_type").and_then(Value::as_str).unwrap_or(""),
        u64::try_from(source_count).map_err(|_| StoreError::Corrupt("negative Artifact source count".to_owned()))?,
        previous
            .get("created_at")
            .and_then(Value::as_str)
    ...
    let canonical = canonical_json(&projection)
        .map_err(|error| StoreError::Corrupt(format!("Artifact projection serialization failed: {error}")))?;
    ...
    transaction.execute(
        "UPDATE artifacts
         ...
         WHERE artifact_id = ?1",
        params![artifact_id, canonical, digest, i64::try_from(revision).map_err(|_| StoreError::Corrupt("Artifact revision out of SQLite range".to_owned()))?, now],
    )?;
    load_artifact_connection(transaction, artifact_id)?
        .ok_or_else(|| StoreError::Corrupt(format!("Artifact {artifact_id} disappeared after update")))
}
...
fn list_stored_artifacts(store: &Store, workspace_id: Option<&str>) -> Result<Vec<StoredArtifact>, ArtifactApiError> {
    let connection = store.connection().map_err(ArtifactApiError::from_store)?;
    ...
    if let Some(workspace_id) = workspace_id {
        let rows = statement
            .query_map([workspace_id], stored_artifact_row)
            .map_err(StoreError::from)
            .map_err(ArtifactApiError::from_store)?;
        for row in rows {
            result.push(row.map_err(StoreError::from).map_err(ArtifactApiError::from_store)?);
        }
    } else {
        let rows = statement
            .query_map([], stored_artifact_row)
            .map_err(StoreError::from)
            .map_err(ArtifactApiError::from_store)?;
        for row in rows {
            result.push(row.map_err(StoreError::from).map_err(ArtifactApiError::from_store)?);
...
fn find_artifact_by_owner_ref(
    ...
    let artifact_id: Option<String> = connection
        ...
        .optional()?;
    match artifact_id {
        Some(artifact_id) => load_artifact_connection(&connection, &artifact_id),
        None => Ok(None),
    }
}
...
fn bounded_text(value: &str, max_chars: usize, field: &str) -> Result<String, StoreError> {
    let value = value.trim();
    if value.chars().count() > max_chars || value.contains(['\r', '\n', '\0']) {
...
fn artifact_filename(requested: &str, source_path: &FsPath) -> Result<String, StoreError> {
    ...
    if candidate.chars().count() > MAX_FILENAME_CHARS
        ...
        || candidate == ".."
    {
        return Err(StoreError::InvalidInput("invalid Artifact filename".to_owned()));
    }
    Ok(candidate)
}
...
mod tests {
    ...
    fn filenames_cannot_escape_or_inject_headers() {
        ...
        assert_eq!(artifact_filename("proof.bin", &source).unwrap(), "proof.bin");
    }

    #[test]
    fn only_lumi_owner_is_accepted_by_phase5_route_contract() {
        assert_ne!("oracle", "lumi");
    }
