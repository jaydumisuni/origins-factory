...
pub fn list_repository_files(
    ...
    if !requested.is_dir() {
        return Err(WorkspaceFileError::invalid(
            "requested repository path is not a directory",
        ));
    }
...
pub fn read_repository_file(
    ...
    if metadata.len() > MAX_FILE_BYTES {
        return Err(WorkspaceFileError::too_large(format!(
            "file is {} bytes; editor limit is {MAX_FILE_BYTES}",
            metadata.len()
        )));
    }
...
pub fn write_repository_file(
    ...
    if bytes.len() as u64 > MAX_FILE_BYTES {
        return Err(WorkspaceFileError::too_large(format!(
            "content is {} bytes; editor limit is {MAX_FILE_BYTES}",
            bytes.len()
        )));
    }
    ...
    if candidate.exists() {
        ...
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(WorkspaceFileError::unsupported(
                "editor writes regular non-symlink files only",
            ));
        }
...
fn repository_root(
    ...
    let repository = store
        .get_repository(repository_id)
        .map_err(WorkspaceFileError::store)?;
    let root = repository["worktree_root"].as_str().ok_or_else(|| {
        WorkspaceFileError::store(StoreError::Corrupt(
            "repository worktree_root missing".to_owned(),
        ))
    })?;
    let canonical = policy
        .authorize_existing_dir(root)
        .map_err(WorkspaceFileError::invalid)?;
    Ok(canonical)
}
    ...
    for component in path.components() {
        match component {
            ...
            Component::ParentDir | Component::RootDir | Component::Prefix(_) => {
                return Err(WorkspaceFileError::invalid(
                    "parent/root/prefix path components are not accepted",
                ));
            }
        }
    }
