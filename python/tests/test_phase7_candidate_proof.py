...
def test_engineering_subject_strips_client_authority_assertions() -> None:
    subject = _engineering_subject(
        _record(),
        {
            "repository_id": "repo-7",
            "task": "upgrade",
            "plan": "plans/upgrade.json",
            "approval_id": "client-supplied",
            "approval_state": "approved",
            "owner_approved": True,
            "decision": "approved",
            "decided_by": "fake-owner",
        },
    )
    for forbidden in ("approval_id", "approval_state", "owner_approved", "decision", "decided_by"):
        assert forbidden not in subject
    assert subject["operation_id"] == "child-7"
    assert subject["apply_plan"] is True
...
def test_candidate_change_proof_requires_real_untruncated_tracked_diff() -> None:
    ...
    proof = _candidate_change_proof(
        before,
        after,
        {"kind": "unstaged", "complete_bytes": 42, "sha256": F, "truncated": False},
    )
    ...
    with pytest.raises(Phase7RuntimeError, match="non-empty tracked repository diff"):
        _candidate_change_proof(
            before,
            after,
            {"kind": "unstaged", "complete_bytes": 0, "sha256": F, "truncated": False},
        )
    with pytest.raises(Phase7RuntimeError, match="truncated"):
        _candidate_change_proof(
            before,
            after,
            {"kind": "unstaged", "complete_bytes": 42, "sha256": F, "truncated": True},
        )
    with pytest.raises(Phase7RuntimeError, match="did not change repository status"):
        _candidate_change_proof(
            before,
            {**after, "status_sha256": D},
            {"kind": "unstaged", "complete_bytes": 42, "sha256": F, "truncated": False},
        )
