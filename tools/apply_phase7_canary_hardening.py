...
OLD_CANARY = '''        gap = _mapping(record, "gap")
...
NEW_CANARY = '''        gap = _mapping(record, "gap")
        candidate = _mapping(record, "candidate")
...
HELPER_ANCHOR = '''def _candidate_change_proof(before: Mapping[str, object], after: Mapping[str, object], diff: Mapping[str, object]) -> dict[str, object]:
'''

HELPER = '''def _validate_canary_binding(
...
def main() -> None:
    ...
    if "def _validate_canary_binding(" not in text:
        if text.count(OLD_CANARY) != 1:
            raise SystemExit(f"canary method patch anchor count={text.count(OLD_CANARY)}")
        if text.count(HELPER_ANCHOR) != 1:
            raise SystemExit(f"helper anchor count={text.count(HELPER_ANCHOR)}")
        text = text.replace(OLD_CANARY, NEW_CANARY, 1)
