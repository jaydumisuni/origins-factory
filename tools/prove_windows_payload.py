...
def prove_builder_copy(project: Path, report_path: Path) -> dict[str, object]:
    ...
    reported_project = Path(str(report.get("project_path", "")))
    if not reported_project.is_dir() or reported_project.resolve() != project_root:
        raise WindowsProofError("Builder report project root does not match the proven candidate")

    package = Path(str(report.get("package_folder", "")))
    payload = Path(str(report.get("payload_folder", "")))
    setup = Path(str(report.get("setup_exe", "")))
    if not package.is_dir() or package.is_symlink():
        raise WindowsProofError(f"Builder package folder unavailable: {package}")
    if not payload.is_dir() or payload.is_symlink():
        raise WindowsProofError(f"Builder payload folder unavailable: {payload}")
    if not setup.is_file() or setup.is_symlink() or setup.stat().st_size <= 0:
        raise WindowsProofError(f"Builder Setup EXE unavailable: {setup}")

    source_root = (project / "dist").resolve()
    package_root = package.resolve()
    payload_root = payload.resolve()
    setup_path = setup.resolve()
    if payload_root == source_root or payload_root.is_relative_to(source_root) or source_root.is_relative_to(payload_root):
        raise WindowsProofError("Builder payload folder must be an independent copy, not the source payload")
    expected_payload_root = (package_root / "Payload" / "Origins Factory").resolve()
    if payload_root != expected_payload_root:
        raise WindowsProofError("Builder payload folder is not bound to the reported package root")
    if setup_path.parent != package_root or setup_path.name != "Origins Factory Setup.exe":
        raise WindowsProofError("Builder Setup EXE is not bound to the reported package root")

    reported_setup_hash = str(report.get("setup_exe_sha256", "")).strip().lower()
    actual_setup_hash = sha256_file(setup)
    if len(reported_setup_hash) != 64 or reported_setup_hash != actual_setup_hash:
        raise WindowsProofError("Builder Setup EXE hash does not match the Builder report")

    source_files = regular_files(project / "dist")
    copied_files = regular_files(payload)
    if report.get("payload_file_count") != len(copied_files):
        raise WindowsProofError("Builder payload file count does not match the copied artifact")
    ...
    return {
        ...
        "setup_exe": str(setup),
        "setup_exe_sha256": actual_setup_hash,
        "signed": False,
        "release_ready": False,
    }
