...
def wait_until(operation: Callable[[], Any], *, timeout: float = 15.0) -> Any:
...
class FixtureHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/v1/lumi":
            return self.send_json(200, {"available": True, "owner": "lumi", "queue_depth": 0, "status": "ready"})
...
def click_button(cdp: Cdp, label: str) -> None:
    ...
    clicked = cdp.evaluate(
        f"(()=>{{const label={label_json};const button=[...document.querySelectorAll('button')].find((item)=>item.textContent?.trim()===label);if(!button)return false;button.click();return true;}})()"
    )
...
def set_phase5_token(cdp: Cdp) -> None:
    token_json = json.dumps(PROOF_TOKEN)
    result = cdp.evaluate(
        f"(()=>{{const input=document.querySelector('.phase5-connect input[type=password]');if(!input)return null;const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;setter.call(input,{token_json});input.dispatchEvent(new Event('input',{{bubbles:true}}));input.dispatchEvent(new Event('change',{{bubbles:true}}));return input.value;}})()"
    )
    if result != PROOF_TOKEN:
        raise RuntimeError("Phase 5 proof token input did not update")
...
def main() -> int:
    ...
    profile = Path(tempfile.mkdtemp(prefix="origins-phase5-chrome-"))
    ...
    with FixtureServer() as fixture:
        try:
            ...
            env = os.environ.copy()
            env["VITE_ORIGINSD_PROXY_TARGET"] = fixture.origin
            env["VITE_ORIGINS_PHASE5_PROXY_TARGET"] = fixture.origin
            ...
            assert_contains(body_text(cdp), "Origins Factory · Phase 5", "Core", "Oracle", "Logistics", "Applications")
            ...
            wait_until(
                lambda: str(cdp.evaluate("document.querySelector('.phase5-status')?.textContent") or "").strip().lower() == "connected",
                timeout=10,
            )
